...
> Sia $(X,\tau)$ uno [[Teoria di Topologia Generale/Topologia|spazio topologico]]. Una famiglia di insiemi $\mathcal{U}(x)\subseteq \tau$ si dice **sistema fondamentale di [[Teoria di Topologia Generale/Intorni|intorni]]** se $\forall A\in \tau\mid x\in A: \exists V\in \mathcal{U}(x)\mid x\in V\subseteq A$
...
> Sia $(X,\tau)$ [[Teoria di Topologia Generale/Topologia|spazio topologico]] e $\beta$ base per $\tau$. Allora $\forall x\in X:\beta_{x}=\{ B\in \beta \mid x\in B \}$ è un sistema fondamentale d'intorni.
