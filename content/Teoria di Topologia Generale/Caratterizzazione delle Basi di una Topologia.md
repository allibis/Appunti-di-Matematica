...
> $\beta$ è una [[Teoria di Topologia Generale/Base di una Topologia|base]] per $\tau$ se verifica le seguenti proprietà:
...
> Viceversa se un'insieme $\beta \subseteq \mathcal{P}(X)$ verifica le proprietà 1. e 2. allora $\exists!\tau$ [[Teoria di Topologia Generale/Topologia|topologia]] su $X$ di cui $\beta$ è base.
^caratt-base-top-en
...
> - Sia $\{U_{i}\}_{i\in I}$ una famiglia di elementi di $\tau$. Allora $\displaystyle \forall U_{i}, \exists \beta_{i}\subseteq \beta \text{ t.c. } U_{i}=\bigcup_{B_{j}\in \beta_{i}} B_{j}$ 
>   Pertanto $\bigcup_{i\in I}U_{i}=\bigcup_{i\in I}\left( \bigcup_{B_{j}\in\beta_{i}}B_{j} \right)$
