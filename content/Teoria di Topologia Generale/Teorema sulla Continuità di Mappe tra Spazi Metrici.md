...
> Siano $(X,d_{X}),(Y,d_{Y})$ due [[Teoria di Topologia Generale/Spazio metrico|spazi metrici]].
> Una funzione $f:X\to Y$ è [[Teoria di Topologia Generale/Mappa Continua|continua]] $\iff \forall\varepsilon>0\,\,\exists \delta>0\, \vert \,d_{X}(x,y)<\delta\implies d_{Y}(f(x),f(y))<\varepsilon$ 
...
> Sia $V$ un [[Teoria di Topologia Generale/Aperto|aperto]] di $Y$. Considero la controimmagine $U=f^{-1}(V)$.
...
> Poiché $V$ è aperto, $\exists \varepsilon>0$ tale che la [[Teoria di Topologia Generale/Palla aperta|palla]] $B_{Y}(f(x),\varepsilon)\subseteq V$.
> 
