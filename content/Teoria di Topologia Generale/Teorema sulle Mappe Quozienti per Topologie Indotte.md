...
> Sia $A\subseteq X$ un qualsiasi insieme e $p:X\to Y$ una [[Teoria di Topologia Generale/Mappa Quoziente|mappa quoziente]].
> Restringo $p$ ad $A$, ottenendo la funzione $(p_{\vert A})_{\#}:A\to p(A)\subseteq Y$
> Non è detto che $(p_{\vert A})_{\#}$ sia mappa quoziente.

> [!teorema] 
> Sia $p:X\to Y$ una [[Teoria di Topologia Generale/Mappa Quoziente|mappa quoziente]] e sia $A$ un sottoinsieme [[Teoria di Topologia Generale/Insieme Saturo rispetto a una Relazione|saturo]] di $X$. Posto $(p_{\vert A})_{\#}=:q:A\to p(A)$, allora
