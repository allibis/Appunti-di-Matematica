...
> Se $\beta$ è [[Base di una Topologia|base]] per $(X,\tau)$, allora $\beta_{Y}=\{ B\cap Y\,\vert\,B\in \beta \}$ è base per il [[Topologia di Sottospazio|sottospazio]] $(Y,\tau_{Y})$

> [!dim]
> Sia $U\subseteq X$ e $y\in U\cap Y$
> Allora per la definizione di [[Base di una Topologia|base]] $\exists B\in \beta \,\vert\,y\in B\cap Y\subseteq U\cap Y$. 
