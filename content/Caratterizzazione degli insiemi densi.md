...
> Allora $F:=\complement_{X}U$ è chiuso in quanto complementare di [[Aperto|aperto]] e $U\not=\varnothing\implies F\subsetneq X$. Quindi ho trovato un [[Chiuso|chiuso]] che contiene $A$ ed è contenuto in $X$. Ma ciò è assurdo perché $\overline{A}=X$. 
> $\impliedby$ :
> Sia $p\in X$. Allora per ipotesi $U\in \tau \mid p\in U: U\cap A\not=\varnothing$. Dunque $p$ è un [[Punti di Aderenza|punto di aderenza]] per $A$ e dunque $p\in \overline{A}$. Concludo che $\overline{A}=X$.
