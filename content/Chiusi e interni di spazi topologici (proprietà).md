...
> 	   $\forall U\in \tau_{Y}$ (ossia $\forall \tilde{U}\in \tau \vert U=\tilde{U}\cap Y$) [[Intorni#^def-intorno|intorno]] di $p$ risulta $U\cap \overline{A}^Y\not=\varnothing$ (perché $p$ è punto [[Interno|interno]] ad $A$).
...
>      Sia $p \in \overline{A}^X\cap Y$ e sia $U\in \tau_{Y} \,\vert\, p\in U$. Allora $\exists \tilde{U}\in \tau\,\vert\,U=\tilde{U}\cap Y$. 
